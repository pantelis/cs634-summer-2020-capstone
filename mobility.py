...
for i in range(1, 2500):
    try:
        if i in dropoff_matrix[:].columns and i in pickup_matrix[:].columns:
            distance_obj[str(i)] = dropoff_matrix[:][i] - pickup_matrix[:][i]
        elif i in dropoff_matrix[:].columns:
            distance_obj[str(i)] = dropoff_matrix[:][i]
        elif i in pickup_matrix[:].columns:
            distance_obj[str(i)] = -pickup_matrix[:][i]

    except KeyError:
        pass


#distance_df is a 24x1010 (hour by zone) matrix containing the net change in a given zone
...
distance_df = pd.DataFrame(distance_obj)
